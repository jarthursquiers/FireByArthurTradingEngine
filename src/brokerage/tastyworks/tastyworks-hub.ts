...
export class TastyworksHub implements IDataLoader {
    ...
    loadPortfolioFromCSV(csvString: string, aPortfolio: Portfolio) {
        ...
        let dataArray = CSVToArray(csvString, ",");
        ...
        for (var i = 1, lenCsv = dataArray.length; i < lenCsv; i++) {
            ...
            wOption.quantity = parseInt(dataArray[i][headerArray.indexOf("Quantity")]);
            wOption.cost = parseFloat(dataArray[i][headerArray.indexOf("Cost")]);
            wOption.deltaPerQty = parseFloat(dataArray[i][headerArray.indexOf("/ Delta")]) * 100;
            wOption.theta = parseFloat(dataArray[i][headerArray.indexOf("Theta")]);
            wOption.netLiq = parseFloat(dataArray[i][headerArray.indexOf("NetLiq")]);
            wOption.callOrPut = dataArray[i][headerArray.indexOf("Call/Put")];
            wOption.strikePrice = parseFloat(dataArray[i][headerArray.indexOf("Strike Price")]);
            wOption.dte = parseInt(dataArray[i][headerArray.indexOf("DTE")].split('d')[0]);

            if (JLog.isDebug()) JLog.debug(`Adding read csv TW opton with symbol ${wOption.symbol} with netliq of ${wOption.netLiq}`);
