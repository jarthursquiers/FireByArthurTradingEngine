...
if (now.valueOf() > startTime.valueOf() && now.valueOf() < endTime.valueOf() ) return true;
else return false;
...
export function showSidebar() {
  ...
  function getTDAService() {
    let tdaClientId = EngineConfig.instance().getConfig(EngineConfigProperty.TDAmeritradeAPIClientId);
    let tdaClientSecret = EngineConfig.instance().getConfig(EngineConfigProperty.TDAmeritradeAPIClientId);
...
lockable_ = function(func) {
 ...
 getValue = function(key) {
   ...
   //Check for a file
   let file = DriveApp.getFilesByName(prefixedKey+"firebyarthurtime.vxx").next();
   if (file != null) {
       value = JSON.parse(file.getBlob().getDataAsString());
       this.memory_[key] = value;
       this.saveToken_(value);
       return value;
   }
